...
| R9 | 扫描阶段（`_remote_stat_tree_fast` 等）不响应取消，9 万文件扫描期点 × 要等扫完 | 中 | ✅ 已修 |
...
### 第三轮（R8 / R9 / 打包杂项）
- **R8 全量同步清理子进程**：`/api/sync` 改用 `subprocess.Popen` + `communicate(timeout=120)`，超时路径调用新增 `_kill_proc_tree()`（Windows 用 `taskkill /PID /T /F` 杀掉进程树，含派生的 SSH 子进程），确保不残留孤儿进程。
